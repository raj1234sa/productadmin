...
if($isAjaxRequest && ($listingData || $export)) {
    ...
    if($export) {
        ...
        echo json_encode(exportReport($spreadsheet, 'export_languages.xlsx'));
        exit;
    }
    ...
    if(!empty($siteLanguages)) {
        ...
        foreach ($siteLanguages as $lang) {
            ...
            if(!empty($flag_name)) {
                ...
                $srcPath = DIR_WS_IMAGES_FLAGS.$flag_name;
            }
            $rec['DT_RowId'] = "language:".$lang['language_id'];
            $rec['language_id'] = $sr++;
            $rec['language_name'] = $lang['language_name'];
            if(!empty($http_path)) {
                $rec['language_name'] .= drawImge($http_path, $srcPath, array('width'=>30, 'class'=>'ml-2'));
            }
